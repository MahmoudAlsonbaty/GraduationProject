...
#define HOR_DIR_PIN  19 //DONE
#define HOR_STEP_PIN 18 //DONE 
#define HOR_ENABLE_PIN 23 //DONE
// Limit switch pins Horizontal (X-axis)
#define HOR_LIMIT_MIN_PIN  14 //DONE
#define HOR_LIMIT_MAX_PIN  27 //DONE

// Motor pins Vertical (Y-axis) 
#define VERT_DIR_PIN  5   //DONE
#define VERT_STEP_PIN 17  //DONE
#define VERT_ENABLE_PIN 22 //DONE
// Limit switch pins Vertical (Y-axis)
#define VERT_LIMIT_MIN_PIN 4 //DONE
#define VERT_LIMIT_MAX_PIN 25 //DONE

// Servo Motors
Servo DOOR_SERVO;  
Servo GRABBER_SERVO; 

#define GRABBER_SERVO_PIN 12 //DONE
#define DOOR_SERVO_PIN 13 //DONE

#define GRABBER_SERVO_IDLE_ANGLE 170 
#define GRABBER_SERVO_GRAB_ANGLE 0

#define DOOR_SERVO_OPEN_ANGLE 90
#define DOOR_SERVO_CLOSED_ANGLE 0

// Speed
#define CALIBRATION_SPEED 1500
#define CALIBRATION_ACCELERATION 1500
...
void moveBothMotor(long horTarget,long VertTarget){
  bool horDir = (horTarget > HOR_Stepper.currentPosition()); //TRUE -> going left to Min
  bool vertDir = (VertTarget > VERT_Stepper.currentPosition());//TRUE -> going up to Min

  //In case they were stopped before
  HOR_Stepper.enableOutputs();
  VERT_Stepper.enableOutputs();
  
  HOR_Stepper.moveTo(horTarget);
  VERT_Stepper.moveTo(VertTarget);

  while (HOR_Stepper.distanceToGo() != 0 || VERT_Stepper.distanceToGo() != 0) {
    //SAFETY
    bool limitHorMin = !digitalRead(HOR_LIMIT_MIN_PIN);
    bool limitHorMax = !digitalRead(HOR_LIMIT_MAX_PIN);
    bool limitVerMin = !digitalRead(VERT_LIMIT_MIN_PIN);
    bool limitVerMax = !digitalRead(VERT_LIMIT_MAX_PIN);

    if((horDir && limitHorMin)/*Going Left to min*/ || (!horDir && limitHorMax) /*Going Right to max*/){
      HOR_Stepper.disableOutputs();
      HOR_Stepper.stop();
      while (HOR_Stepper.isRunning()) HOR_Stepper.run();
      Serial.println("Stopping Movement as we reached a Horizontal Limit");
      break;
    }
    if((vertDir && limitVerMin)/*Going Left to min*/ || (!horDir && limitVerMax) /*Going Right to max*/){
      VERT_Stepper.disableOutputs();
      VERT_Stepper.stop();
      while (VERT_Stepper.isRunning()) VERT_Stepper.run();
      Serial.println("Stopping Movement as we reached a Vertical Limit");
      break;
    }
...
void calibrateBoth(){
    ...
    delay(500); // Wait for a second to ensure everything is stable
    Serial.println("Retrackting a bit");
    ...
    //Retraction
    moveBothMotor(HOR_maxPosition/2,VERT_maxPosition/2);
}
...
void setup() {
  Serial.begin(9600);
  pinMode(HOR_LIMIT_MIN_PIN, INPUT_PULLUP);
  pinMode(HOR_LIMIT_MAX_PIN, INPUT_PULLUP);
  pinMode(VERT_LIMIT_MIN_PIN, INPUT_PULLUP);
  pinMode(VERT_LIMIT_MAX_PIN, INPUT_PULLUP);
...
void GO(String command) {
      ...
      moveBothMotor(xPos,yPos);


}

void setData(String setCmd){
...
void loop() {
  //Here we check te serial monitor for what's next to do
  if (Serial.available()) {
    ...
    //
      ...
      timeSinceLastLimitSwitchTest = millis();
      testLimitSwitches();
    }
  }
}
...
void dropOFF(){
  ...
  moveBothMotor(HOR_DROP_OFF_POSITION,VERT_DROP_OFF_POSITION);
...
long columnToPos(int column){
  ...
  case 7:
    ...
    Serial.println("Column 7");
    Serial.println(pos);
    break;
  case 8:
    pos = COLUMN_8;
    
    Serial.println("Column 8");
    Serial.println(pos);
    break;
  default:
    break;
