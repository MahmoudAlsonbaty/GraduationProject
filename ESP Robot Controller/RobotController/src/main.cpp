...
void calibrateBoth(){
    ...
    horLimitMin = !digitalRead(HOR_LIMIT_MIN_PIN);
    vertLimitMin = !digitalRead(VERT_LIMIT_MIN_PIN);
    HOR_Stepper.moveTo(HOR_maxPosition /2); // Retract a bit from max position
    VERT_Stepper.moveTo(VERT_maxPosition /2);
    
  while (HOR_Stepper.distanceToGo() != 0 || VERT_Stepper.distanceToGo() != 0) {
    //SAFETY
    // if (digitalRead(HOR_LIMIT_MIN_PIN) == LOW || digitalRead(HOR_LIMIT_MAX_PIN) == LOW) {
    //   HOR_Stepper.disableOutputs();
    // }else{
    //   HOR_Stepper.enableOutputs();
    // }

    // if( digitalRead(VERT_LIMIT_MIN_PIN) == LOW || digitalRead(VERT_LIMIT_MAX_PIN) == LOW) {
...
void loop() {
  ...
  bool horStateChanged = false;
  bool vertStateChanged = false;
